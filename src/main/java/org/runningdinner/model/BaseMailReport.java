 ...
 * It holds mainly the information about the sending state and the information about the failed and/or succeeded mails that have been sent.<br>
 * <br>
 * Note: Even if all mail addresses are marked as being successful sent, this must not be true, as we can just track whether we could submit
 * the email messages to the concrete mail server.
 * 
...
public abstract class BaseMailReport extends AbstractEntity {
	...
	/**
	 * Indicates whether there this job for sending the mails was interrupted (e.g. by container shutdown)
	 * 
	 * @return
	 */
	...
	/**
	 * Indicates that the sending task is finished now and passes all mail addresses with a boolean flag (successful/failed) as a result.
	 * 
	 * @param mailAddressStatusMappings
	 */
	public void applySendingFinished(Map<String, Boolean> mailAddressStatusMappings) {
		this.sending = false;
	...
	/**
	 * Returns the status of all sent emails as a map with each email as key and a boolean indicating success or failure.
	 * 
	 * @return
	 */
	...
	protected Set<String> filterMails(boolean status) {
		if (mailAddressStatusMapping == null || mailAddressStatusMapping.size() == 0) {
	...
	public String toString() {
		return "Report-Status: " + isSending() + "; Sending-Start: "
				+ CoreUtil.getFormattedTime(getSendingStartDate(), CoreUtil.getDefaultDateFormat(), "Unknown");
	}
